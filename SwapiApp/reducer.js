  ...
  return async dispatch => {
    try {
      const res = await axios.get(
        `${baseUrl}/${section}/?search=${searchQuery}`
      );
      //create action
      ...
      if (section === 'people') {
        ...
        res.data.results.map(async person => {
          const starships = await Promise.all(
            promisifiedDataHelper(person.starships)
          );
        ...
        res.data.results.map(async planet => {
          const residents = await Promise.all(
            promisifiedDataHelper(planet.residents)
          );
          dispatch(getTertiary(residents));
        });
      } else {   // section === starships  
        res.data.results.map(async starship => {
          const films = await Promise.all(
            promisifiedDataHelper(starship.films)
          );
          dispatch(getSecondary(films));
        });
        res.data.results.map(async starship => {
          const pilots = await Promise.all(
            promisifiedDataHelper(starship.pilots)
          );
